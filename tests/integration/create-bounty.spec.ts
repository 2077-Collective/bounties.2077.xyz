...
import { setupDb, createUserFixture, clearSponsorsTable, createSponsorFixture } from './utils/fixtures';
...
test.describe.only('Create Bounty Form', () => {
  ...
  test('fills out the form and submits successfully', async ({ cookiePage }) => {
    ...
    await cookiePage.fill('textarea[name="description"]', 'Detailed description of the test bounty');
    ...
    // Check if redirected to sponsor dashboard
    await expect(cookiePage).toHaveURL('/app/sponsor');

    // Verify the new bounty is listed on the sponsor dashboard
    await expect(cookiePage.getByText('Test Bounty')).toBeVisible();
  });

  test.skip('displays validation errors for empty required fields', async ({ cookiePage }) => {
    await cookiePage.click('button[type="submit"]');

    await expect(cookiePage.getByText('Title is required')).toBeVisible();
    await expect(cookiePage.getByText('Excerpt is required')).toBeVisible();
    await expect(cookiePage.getByText('Briefing is required')).toBeVisible();
    await expect(cookiePage.getByText('Start Date is required')).toBeVisible();
    await expect(cookiePage.getByText('End Date is required')).toBeVisible();
    await expect(cookiePage.getByText('Reward amount is required')).toBeVisible();
  });
});
