...
describe.skip('Add new user to waitlist', () => {
	...
	test('return the waitlist ranked by most order of insertion and referral count', async () => {
		const { referralCode: test0ReferralCode } = await enterWaitlist(testEmail0, undefined, tx);
		const { referralCode: test1ReferralCode } = await enterWaitlist(testEmail1, undefined, tx);
	...
	test('should throw an error if the email is already in the waitlist', async () => {
		await enterWaitlist(testEmail0, undefined, tx);
		await expect(enterWaitlist(testEmail0, undefined, tx)).rejects.toThrowError(
