<script lang="ts">
	...
	const submitFunction: SubmitFunction = (event) => {
		return enhanceSubmit(requiredSkills, paymentChain, tokenId, rewards)(event);
	};
</script>

<div class="container mx-auto px-4 py-8">
	<CardContent>
		<form method="POST" {action} use:enhance={submitFunction} class="space-y-6 max-w-2xl mx-auto">
			...
			<div>
				<Button type="submit">{bounty ? 'Update bounty' : 'Create bounty'}</Button>
			</div>
		</form>
	</CardContent>
</div>
